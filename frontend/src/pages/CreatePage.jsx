...
const CreatePage = () => {
  ...
  return (
    ...
    <span className='label-text'>Year of Publishing</span>
     ...
     <input text="text" placeholder='e.g. 2025' className='input input-bordered' value={publishYear} onChange={(e) =>
      setPublishYear(e.target.value)} required />
      </div>
    <div className='card-actions jsutify-end'>
      <button type='submit' className='btn btn-primary' disabled={loading}>{loading ? "Creating..." : "Create Book"}</button>    
      </div> </form>
      </div> </div> </div> </div> </div>
  )
}
